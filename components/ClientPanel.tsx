...
interface ClientPanelProps {
  ...
  nextDrawTime: string; // Label text
  nextDrawTarget: Date | null; // Real date for countdown
  isSyncing: boolean;
  transactions: Transaction[];
}

type NewTicket = Omit<Ticket, 'id' | 'purchaseDate' | 'status'>;
type Tab = 'play' | 'wallet';
...
const ClientPanel: React.FC<ClientPanelProps> = ({ 
  ...
  // --- COUNTDOWN LOGIC ---
  useEffect(() => {
      if (!nextDrawTarget) return;

      const calculateTime = () => {
          const now = new Date().getTime();
          const target = new Date(nextDrawTarget).getTime();
    ...
    <div className="space-y-8 relative">
        ...
        <div className="relative z-10 p-6 md:p-8">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 gap-4">
                ...
                <div className="text-right hidden md:block">
                     ...
                     <div className="bg-black/30 rounded-lg px-3 py-1.5 border border-brand-accent/30 inline-flex flex-col items-end">
                         <div className="text-2xl font-mono font-black text-brand-accent flex items-center justify-end gap-2 tracking-widest">
                            <ClockIcon className="h-5 w-5 animate-spin-slow opacity-70"/>
                            {timeLeft}
                         </div>
                         <div className="text-[9px] text-brand-text-secondary font-mono uppercase opacity-70 mt-0.5">
                            {nextDrawTime}
                         </div>
                     </div>
                </div>
      ...
      <nav className="flex justify-center mb-4">
          <div className="bg-brand-secondary/80 backdrop-blur-md p-1.5 rounded-2xl border border-brand-border inline-flex gap-2 shadow-2xl">
              <button onClick={() => setActiveTab('play')} className={`px-6 py-3 rounded-xl text-sm font-bold uppercase tracking-wide transition-all flex items-center gap-2 ${activeTab === 'play' ? 'bg-brand-accent text-white shadow-[0_0_20px_rgba(79,70,229,0.4)]' : 'text-brand-text-secondary hover:text-white hover:bg-white/5'}`}>
                  <TicketIcon className="h-4 w-4" /> Jugar
              </button>
              <button onClick={() => setActiveTab('wallet')} className={`px-6 py-3 rounded-xl text-sm font-bold uppercase tracking-wide transition-all flex items-center gap-2 ${activeTab === 'wallet' ? 'bg-brand-accent text-white shadow-[0_0_20px_rgba(79,70,229,0.4)]' : 'text-brand-text-secondary hover:text-white hover:bg-white/5'}`}>
                  <WalletIcon className="h-4 w-4" /> Mi Billetera
              </button>
          </div>
      </nav>
      ...
      {activeTab === 'wallet' && (
          <div className="max-w-4xl mx-auto space-y-6 animate-fade-in-up">
              {/* Wallet Header */}
              <Card className="bg-gradient-to-r from-brand-secondary to-brand-primary border-brand-accent/30">
                  <div className="flex flex-col md:flex-row items-center justify-between gap-6">
                      <div className="flex items-center gap-4">
                           <div className="w-16 h-16 bg-brand-accent/10 rounded-full flex items-center justify-center border-2 border-brand-accent/30 shadow-[0_0_30px_rgba(79,70,229,0.2)]">
                               <WalletIcon className="h-8 w-8 text-brand-accent" />
                           </div>
                           <div>
                               <h2 className="text-3xl font-black text-white">Mi Billetera</h2>
                               <p className="text-brand-text-secondary text-sm">Gestiona tu saldo y revisa tus premios</p>
                           </div>
                      </div>
                      <div className="text-right bg-black/30 p-4 rounded-xl border border-brand-border">
                          <p className="text-xs text-brand-text-secondary uppercase font-bold tracking-wider mb-1">Saldo Disponible</p>
                          <p className="text-4xl font-black font-mono text-white">{formatCurrency(user.balance)}</p>
                      </div>
                  </div>
              </Card>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="md:col-span-2 space-y-4">
                      <h3 className="text-sm font-bold text-white uppercase tracking-widest flex items-center gap-2">
                          <ClockIcon className="h-4 w-4"/> Historial de Movimientos
                      </h3>
                      <div className="bg-brand-secondary border border-brand-border rounded-xl overflow-hidden">
                          {transactions && transactions.length > 0 ? (
                                      ...
                                      <div key={tx.id} className="p-4 flex items-center justify-between hover:bg-white/5 transition-colors">
                                          <div className="flex items-center gap-4">
                                              <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
                                                  ...
                                                  tx.type === 'winnings' ? 'bg-yellow-500/20 text-yellow-400' : 
                                                  tx.type === 'withdraw' ? 'bg-red-500/20 text-red-400' : 
                                                  'bg-brand-tertiary text-brand-text-secondary'
                                              }`}>
                                                  {tx.type === 'deposit' && <ArrowTrendingUpIcon className="h-5 w-5"/>}
                                                  {tx.type === 'winnings' && <FireIcon className="h-5 w-5"/>}
                                                  {tx.type === 'withdraw' && <ArrowTrendingDownIcon className="h-5 w-5"/>}
                                                  {tx.type === 'purchase' && <TicketIcon className="h-5 w-5"/>}
                                              </div>
                                              <div>
                                                  <p className="text-sm font-bold text-white capitalize">
                                                      {tx.type === 'deposit' ? 'Recarga de Saldo' : 
                                                       tx.type === 'winnings' ? 'Premio Acreditado' :
                                                       tx.type === 'withdraw' ? 'Retiro de Fondos' :
                                                       'Compra de Tiquetes'}
                                                  </p>
                                          ...
                                          <div className="text-right">
                                              <p className={`font-mono font-bold ${
                                                  tx.type === 'purchase' || tx.type === 'withdraw' ? 'text-white' : 'text-brand-success'
                                              }`}>
                                                  {tx.type === 'purchase' || tx.type === 'withdraw' ? '-' : '+'}
                                                  {formatCurrency(tx.amount)}
                                              </p>
                                              {tx.details && (
                                                  <p className="text-[10px] text-brand-text-secondary font-mono mt-1 max-w-[250px] leading-tight break-words whitespace-pre-wrap opacity-80">
                                                      {tx.details}
                                                  </p>
                                              )}
                                          </div>
                              ...
                              <div className="p-8 text-center text-brand-text-secondary text-sm italic">
                  ...
                  <div className="space-y-4">
                      <Card className="bg-brand-tertiary/30 border-brand-border">
                          <h4 className="text-xs font-bold text-brand-text-secondary uppercase mb-4">Resumen Rápido</h4>
                          <div className="space-y-4">
                              <div className="flex justify-between items-center">
                                  <span className="text-sm text-white">Total Jugado</span>
                                  <span className="font-mono text-brand-text-secondary">
                                      {formatCurrency(transactions.filter(t => t.type === 'purchase').reduce((acc, t) => acc + t.amount, 0))}
                                  </span>
                              </div>
                              <div className="flex justify-between items-center">
                                  <span className="text-sm text-white">Total Ganado</span>
                                  <span className="font-mono text-brand-success font-bold">
                                      {formatCurrency(transactions.filter(t => t.type === 'winnings').reduce((acc, t) => acc + t.amount, 0))}
                                  </span>
                              </div>
                              <div className="h-px bg-brand-border"></div>
                              <div className="p-3 bg-brand-primary/50 rounded-lg text-xs text-brand-text-secondary leading-relaxed">
                                  <span className="font-bold text-brand-accent">Nota:</span> Para realizar recargas o retiros, por favor contacte a su administrador asignado o utilice los canales oficiales de WhatsApp.
                              </div>
                          </div>
                      </Card>
                  </div>
